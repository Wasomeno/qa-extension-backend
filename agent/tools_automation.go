...
func analyzeTestCase(ctx tool.Context, input AnalyzeTestCaseInput) (*AnalyzeTestCaseOutput, error) {
	...
	log.Printf("[AutomationAgent] Analysis: route=%v, confidence=%.0f%%", output.TargetRoutes, output.Confidence*100)
	return output, nil
}
...
func decideFilesToFetch(ctx tool.Context, input DecideFilesInput) (*DecideFilesOutput, error) {
	...
	log.Printf("[AutomationAgent] Files to fetch: %v", output.FilesToFetch)
	return output, nil
}
...
func fetchSourceFiles(ctx tool.Context, input FetchFilesInput) (*FetchFilesOutput, error) {
	...
	log.Printf("[AutomationAgent] Fetched %d/%d files, %d tokens", len(output.Files), len(input.FilePaths), output.TotalTokens)
	return output, nil
}
...
func extractSelectorsFromFiles(ctx tool.Context, input ExtractSelectorsInput) (*ExtractSelectorsOutput, error) {
	...
	log.Printf("[AutomationAgent] Extracted %d selectors", len(output.Selectors))
	return output, nil
}
...
type BuildAutomationInput struct {
	TestCaseID string         `json:"testCaseID"`
	ScenarioID string         `json:"scenarioID"`
	Selectors  []SelectorInfo `json:"selectors"`
...
func buildAutomationSteps(ctx tool.Context, input BuildAutomationInput) (*BuildAutomationOutput, error) {
	...
	output := &BuildAutomationOutput{
		Automation: &models.GeneratedAutomation{
	...
	if input.AuthConfig.LoginURL != "" {
		output.Automation.Steps = append(output.Automation.Steps,
	...
	for i, step := range testCase.Steps {
		...
		automationStep := models.RecordingStep{
			Action:             action,
		...
		if selector != "" {
			automationStep.Selector = selector
			...
			if sel := getBestSelectorFromMap(selectorMap, action, step.Action, step.InputData); sel != nil {
				automationStep.ElementHints = sel.ToElementHints()
				automationStep.XPath = sel.ToXPath()
				automationStep.XPathCandidates = sel.ToXPathCandidates()
			}
		} else {
		...
		if action == "assert" || strings.Contains(strings.ToLower(step.Action), "verify") || strings.Contains(strings.ToLower(step.Action), "check") {
			automationStep.AssertionType = "visible"
			automationStep.ExpectedValue = step.ExpectedResult
		}

		output.Automation.Steps = append(output.Automation.Steps, automationStep)
	}

	output.Automation.Name = fmt.Sprintf("[%s] %s", testCase.ID, testCase.Name)
	output.Automation.Description = fmt.Sprintf("Auto-generated from %s. %s", input.ScenarioID, testCase.PreCondition)

	log.Printf("[AutomationAgent] Built automation with %d steps", len(output.Automation.Steps))
	return output, nil
}
...
type GenerateAutomationInput struct {
	ScenarioID string     `json:"scenarioID"`
	TestCaseID string     `json:"testCaseID"`
...
type GenerateAutomationOutput struct {
	Automation *models.GeneratedAutomation `json:"automation"`
	Warnings   []string                    `json:"warnings"`
	Issues     []string                    `json:"issues"`
	Confidence float64                     `json:"confidence"`
...
func generateAutomationForTestCase(ctx tool.Context, input GenerateAutomationInput) (*GenerateAutomationOutput, error) {
	...
	buildOutput, err := buildAutomationSteps(ctx, BuildAutomationInput{
		TestCaseID: input.TestCaseID,
		ScenarioID: input.ScenarioID,
		Selectors:  selectors,
	...
	if err != nil {
		return nil, fmt.Errorf("automation build failed: %w", err)
	}
	...
	issues := []string{}
	for i, step := range buildOutput.Automation.Steps {
		if step.Action != "navigate" && step.Selector == "" {
			issues = append(issues, fmt.Sprintf("Step %d ('%s') has no selector", i+1, step.Description))
		}
	}

	log.Printf("[AutomationAgent] Generated automation with %d steps, %d warnings", len(buildOutput.Automation.Steps), len(allWarnings))

	return &GenerateAutomationOutput{
		Automation: buildOutput.Automation,
		Warnings:   allWarnings,
		Issues:     issues,
		Confidence: analysis.Confidence,
		StepsUsed:  len(buildOutput.Automation.Steps),
	}, nil
}
...
type GenerateAutomationsInput struct {
	ScenarioID  string     `json:"scenarioID"`
...
// =============================================================================
// AGENT-BASED AUTOMATION TEST GENERATION
...
func RunAgentForTestGeneration(ctx context.Context, input AutomationAgentInput, token *oauth2.Token) (*GenerateAutomationsOutput, error) {
	log.Printf("[AutomationAgent] RunAgentForTestGeneration: %s", input.ScenarioID)
	...
	log.Printf("[AutomationAgent] Processing %d test cases", output.TotalCount)
	...
	for _, tc := range targetCases {
		...
		output.Automations = append(output.Automations, *automation)
		output.SuccessCount++
		output.Warnings = append(output.Warnings, warnings...)
	}

	log.Printf("[AutomationAgent] Completed: %d/%d automations", output.SuccessCount, output.TotalCount)
	return output, nil
}
...
// agentGenerateSingleAutomation uses GitLab tools to figure out what files to fetch
// based on the test case steps content
func agentGenerateSingleAutomation(ctx context.Context, glClient *gitlab.Client, projectID, scenarioID, branch string, tc *models.ParsedTestCase) (*models.GeneratedAutomation, []string, error) {
	log.Printf("[AutomationAgent] agentGenerateSingleAutomation: %s - %s", tc.ID, tc.Name)
	...
	log.Printf("[AutomationAgent] Step context for %s: %s", tc.ID, stepContext)
	...
	if authConfig.LoginURL != "" {
		automation.Steps = append(automation.Steps,
	...
	for i, step := range normalizeSteps(tc.Steps) {
		...
		automation.Steps = append(automation.Steps, automationStep)
	}

	return automation, warnings, nil
}
...
func agentFetchRelevantFilesFromContext(ctx context.Context, glClient *gitlab.Client, projectID, branch, testCaseName, stepContext string) ([]FetchedFile, []string) {
	...
	for _, path := range searchPaths {
		...
		fetched[path] = true
		log.Printf("[AutomationAgent] Fetched: %s", path)
	}
		...
		for _, kw := range keywords {
			...
			if strings.Contains(nodeName, kwLower) || strings.Contains(kwLower, nodeName) {
				...
				if err == nil {
					...
					log.Printf("[AutomationAgent] Found module: %s", node.Name)
				}
...
func buildGenerationPrompt(scenario *models.TestScenario, sheetNames []string) string {
	...
	var lines []string
	lines = append(lines, "You are generating automation tests from a test scenario.")
	lines = append(lines, "")
...
func generateAutomationsForScenario(ctx tool.Context, input GenerateAutomationsInput) (*GenerateAutomationsOutput, error) {
	...
	for i := 0; i < len(targetCases); i += batchSize {
		...
		for _, tc := range batch {
			result, err := generateAutomationForTestCase(ctx, GenerateAutomationInput{
				ScenarioID: input.ScenarioID,
				TestCaseID: tc.ID,
				AuthConfig: input.AuthConfig,
				ProjectID:  input.ProjectID,
			...
			if result.Automation != nil {
				output.Automations = append(output.Automations, *result.Automation)
				output.SuccessCount++
