...
// GLMAdapter adapts an OpenAI-compatible Vertex AI MaaS endpoint to the ADK LLM interface.
type GLMAdapter struct {
	client *openai.Client
	model  string
}

// NewGLMModel initializes a new GLM adapter using Google Default Credentials
// for Vertex AI Model as a Service.
func NewGLMModel(ctx context.Context, projectID, location, modelName string) (*GLMAdapter, error) {
...
func (g *GLMAdapter) GenerateContent(ctx context.Context, req *model.LLMRequest, stream bool) iter.Seq2[*model.LLMResponse, error] {
	return func(yield func(*model.LLMResponse, error) bool) {
		...
		streamResp, err := g.client.CreateChatCompletionStream(ctx, streamReq)
		if err != nil {
			yield(nil, err)
			return
		}
